...
class ParticleFiler():
    ...
    def __init__(self):
        ...
        self.pose_pub = rospy.Publisher(
            "/pf/viz/inferred_pose", PoseStamped, queue_size=1)
    ...
    def publish_tf(self, pose, stamp=None):
        ...
        # self.sub_tf.waitForTransform("laser", "odom", rospy.Time(),
        # rospy.Duration(3)) # (unnecessary?) slowdown
        laser_odom_transform = self.sub_tf.lookupTransform(
            "laser", "odom", rospy.Time())
        laser_odom_matrix = tf.transformations.compose_matrix(
            translate=laser_odom_transform[0],
            angles=tf.transformations.euler_from_quaternion(
                laser_odom_transform[1]))

        # Calculate map -> odom transform.
        map_odom_matrix = tf.transformations.concatenate_matrices(
            map_laser_matrix, laser_odom_matrix)
        # map_odom_matrix = tf.transformations.concatenate_matrices(laser_odom_matrix, map_laser_matrix)
        _, _, map_odom_rotation_euler, map_odom_pos, _ = tf.transformations.decompose_matrix(
            map_odom_matrix)
        map_odom_rotation = tf.transformations.quaternion_from_euler(
            map_odom_rotation_euler[0],
            map_odom_rotation_euler[1],
            map_odom_rotation_euler[2])

        if stamp is None:
            stamp = rospy.Time.now()

        # not even the worst hack in this function
        stamp = stamp + rospy.Duration(0.2)

        # Publish transform
        self.pub_tf.sendTransform(
            map_odom_pos.tolist(),
            map_odom_rotation.tolist(),
            stamp,
            "odom",
            "map")
    ...
    def visualize(self):
        ...
        if self.particle_pub.get_num_connections() > 0:
            # publish a downsampled version of the particle distribution to
            # avoid a lot of latency
        ...
        if self.pub_fake_scan.get_num_connections(
        ) > 0 and isinstance(self.ranges, np.ndarray):
            # generate the scan from the point of view of the inferred position
            # for visualization
            self.viz_queries[:, 0] = self.inferred_pose[0]
            self.viz_queries[:, 1] = self.inferred_pose[1]
            self.viz_queries[:, 2] = self.downsampled_angles + \
                self.inferred_pose[2]
            self.range_method.calc_range_many(
                self.viz_queries, self.viz_ranges)
    ...
    def initialize_particles_pose(self, pose):
        ...
        self.particles[:, 0] = pose.position.x + \
            np.random.normal(loc=0.0, scale=0.5, size=self.MAX_PARTICLES)
        self.particles[:, 1] = pose.position.y + \
            np.random.normal(loc=0.0, scale=0.5, size=self.MAX_PARTICLES)
        self.particles[:, 2] = Utils.quaternion_to_angle(
            pose.orientation) + np.random.normal(loc=0.0, scale=0.4, size=self.MAX_PARTICLES)
    ...
    def precompute_sensor_model(self):
        ...
        for d in range(table_width):
            ...
            for r in range(table_width):
                ...
                prob += z_hit * \
                    np.exp(-(z * z) / (2.0 * sigma_hit * sigma_hit)) / (sigma_hit * np.sqrt(2.0 * np.pi))

                # observed range is less than the predicted range - short
                # reading
                if r < d:
    ...
    def sensor_model(self, proposal_dist, obs, weights):
        ...
        if self.first_sensor_update:
            ...
            self.tiled_angles = np.tile(
                self.downsampled_angles, self.MAX_PARTICLES)
        ...
        if self.RANGELIB_VAR == VAR_RADIAL_CDDT_OPTIMIZATIONS:
            ...
            if self.SHOW_FINE_TIMING:
                t_start = time.time()
            # this version demonstrates what this would look like with
            # coordinate space conversion pushed to rangelib
            self.queries[:, :] = proposal_dist[:, :]
            if self.SHOW_FINE_TIMING:
                t_init = time.time()
            ...
            if self.SHOW_FINE_TIMING and self.iters % 10 == 0:
                print("sensor_model: init: ", np.round((t_init -
                                                       ...
                                                       t_total, 2), "range:", np.round((t_range -
                                                                                        t_init) /
                                                                                       t_total, 2), "eval:", np.round((t_eval -
                                                                                                                       t_range) /
            ...
            # construct this redundant array
            ...
            # this version directly uses the sensor model in Python, at a
            ...
            # resolve the sensor model by discretizing and indexing into the
            # precomputed table
        ...
        if self.SHOW_FINE_TIMING and self.iters % 10 == 0:
            print("MCL: propose: ", np.round((t_propose -
                                             ...
                                             t_total, 2), "motion:", np.round((t_motion -
                                                                               t_propose) /
                                                                              t_total, 2), "sensor:", np.round((t_sensor -
                                                                                                                t_motion) /
    ...
    def expected_pose(self):
        # returns the expected value of the pose given the particle
        # distribution
    ...
    def update(self):
        ...
        if self.lidar_initialized and self.odom_initialized and self.map_initialized:
            if self.state_lock.locked():
                ...
                t1 = time.time()
                observation = np.copy(
                    self.downsampled_ranges).astype(
                    np.float32)
